...
**This repository does Problem A — health-insurance claim first response.** Problem B's
tools, script, routing rules, notebook and data have been stripped out, so `PROBLEM = "A"`
in `config.py` is fixed rather than a choice.

**It is a starting point, not a solution.** It handles one case and stops. Everything it
does is something you will replace with your own design.

**Start with `A2_Scaffold_Tour_ProblemA.ipynb`** — it walks the whole machine on one case,
with every value printed. Then work in the modules.
...
```bash
cd A2_scaffold
python3 run_eval.py CLM-8842          # one case, every turn shown
...
**If you edit `config.py` and nothing happens**, Python is reusing stale bytecode — an
edit that leaves the file the same size, such as `AUTONOMY = "confirm"` → `"suggest"`,
can slip past its cache check. The scaffold detects this and prints a loud warning; the
fix is `rm -rf __pycache__`, or restart the kernel in a notebook. (This bit us while
building the scaffold, which is why there is a guard for it.)
...
Every value printed is real. Wherever a cell hard-codes something — `'M-2214'`,
`'POL-3310'`, `'62480'` — the markdown says which earlier cell it came from, so nothing
appears by magic.
...
**Cell 5** runs four duplicate-matching strategies and shows that every shortcut wrongly
escalates a perfectly good claim — including this one.

### What it is not

**It contains no logic of its own.** Every cell imports from the `.py` files. That is
...
**It is not what you submit.** D5(a) says a marker clones your repository and reproduces
...
whether the agent makes another call. A one-line claim on a lapsed policy ends the
run early. You did not write those branches; the record did.

**4 · Only independent calls fold into one turn.** `CLM-8842` is 8 calls in 4
turns, which matches Appendix A exactly — check it. A dependency chain cannot be
shortened by running things at once: the pre-authorisation lookup cannot join the
coverage turn, because coverage is what says an authorisation is needed. Even so,
Problem A saves 54%.
...
1. **Open the tour notebook** and run it top to bottom. Fifteen minutes, and you will
   have seen the whole machine on one case.
2. **Open `config.py`** and read it — the guardrail limits and the autonomy setting are
   the ones you will be defending.
...
- **Only one case is scripted.** Your set needs 30–50. See
...
- Your extended `expected_outcomes_A.json` is committed. **A pass rate submitted
