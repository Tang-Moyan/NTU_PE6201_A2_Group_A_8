...
SECTION_4_PROSE = """We use the Class 5 three-layer model on measured numbers from the D5 live battery. Layer 1 is 8,891 input and 1,019 output tokens per run at OpenRouter list prices for google/gemini-3.8-flash (US$0.75/US$3.75 per million, verified 2026-09-15 against openrouter.ai/api/v1/models, our procurement path): US$0.0105. Layer 2 is (1 - 0.506) x US$7.60 = US$3.75: 50.6% is our measured pass rate over 81 live trials, US$7.60 Appendix A's assessor at US$38/hour for 12 minutes. Layer 2 is 358 times Layer 1 - that ratio, not the token bill, is the finding. Layer 3 is US$73 a month, amortising forever.

At 8,000 claims a month the total is US$30,182: US$84 of tokens, US$30,025 of escalations, US$73 fixed; US$7.45 per successful task. At 50.6% the agent is not shippable, and the cost model is what makes that visible.

Four levers, measured before and after. (1) Tool block: 1,167 to 1,098 tokens, cutting lookup_hospital, which changed what the record must say, not the decision. (2) Turn count: 215 to 128 turns and 1,287,600 to 617,400 input tokens across 45 cases, by parallelising independent checks; tokens go as B*T + D*T^2/2, so T is the quadratic term and the largest token lever. (3) Observation size: get_preauthorisation returned 296 tokens per call in v1, 21 in v2; run live, v2 scored 50.6% against v1's 43.2%, and 29.4% against 11.8% on the pre-authorisation families - small n, but sitting where the mechanism predicts. (4) Success rate: 33.3% on gpt-4o-mini to 77.8% on glm-5.3, same commit, same prompt, same cases. Lever 4 dominates: one point of pass rate is worth US$608 a month, seven times the whole token bill.

Break-even asks how accurate the cheap model must be to be worth choosing. With C = US$0.00113 on gpt-4o-mini, E = US$3.7636 on gemini-3.8-flash (its own Layer 1 plus Layer 2, not divided by P again - that reading reproduces the brief's 91.2% worked answer, not 90.7%) and F = US$7.60, break-even = 1 - (E - C)/F = 50.5%. The cheap model measured 33.3%, 17.2 points short. It is 9.3 times cheaper per run, but the US$0.0094 it saves is repaid by failing 0.12% more often - which is why break-even lands almost exactly on the expensive model's own pass rate. At US$0.10 per escalation it would be -8.9% and the cheap model would always win. What decides the model is not its price but the price of being wrong.

Across 40.6% to 60.6%, cost per successful task spans US$11.14 to US$4.95: human fallback dominates the token bill at every point in the range. Caps: an 8-turn step cap and a 60,000-token ceiling, both enforced in Guardrails; and 200 claims per account per month - a policy commitment at provisioning, not a code control, since a per-run guard cannot see cross-run usage. Baseline uses API-returned token counts at list prices; no caching or reasoning adjustment."""
