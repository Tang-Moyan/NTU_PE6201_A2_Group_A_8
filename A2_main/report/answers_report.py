...
SECTION_4_PROSE = """Our cost model is the Class 5 three-layer form, built on measured numbers: token counts and pass rates from the D5 live battery, not estimates. Layer 1, the per-task variable cost, is 8,891 input and 1,019 output tokens per run at OpenRouter list prices for google/gemini-3.8-flash (US$0.75/US$3.75 per million, verified 2026-09-15 against openrouter.ai/api/v1/models, our actual procurement path) - US$0.0105 per run. Layer 2, the expected fallback, is (1 - 0.506) x US$7.60 = US$3.75, where 50.6% is our measured live pass rate over 81 trials and US$7.60 is Appendix A's claims assessor at US$38/hour for 12 minutes. Layer 2 is 358 times Layer 1. That ratio, not the token bill, is the finding. Layer 3 is US$73 a month in storage, monitoring, our own evaluation runs and amortised maintenance.

At Problem A's stated volume of 8,000 claims a month the total is US$30,182, of which tokens are US$84 and escalations are US$30,025. Cost per successful task is US$7.44. We report this plainly: at a 50.6% pass rate the agent is not shippable, and the cost model is what makes that visible.

Four levers, each measured before and after. (1) Tool block size: 1,167 to 1,098 tokens by cutting lookup_hospital, whose result changed what the record must say, not the decision. (2) Turn count: 215 to 128 turns and 1,287,600 to 617,400 input tokens across 45 cases by parallelising independent checks; input tokens go as B*T + D*T^2/2, so T is the quadratic term and the largest token lever. (3) Observation size: get_preauthorisation returned 296 tokens per call in v1 and 21 in v2. Run live on one fixed model, v2 also scored 50.6% against v1's 43.2%, and 29.4% against 11.8% on the four pre-authorisation families - small n, but the effect sits where the mechanism predicts. (4) Success rate: 33.3% on gpt-4o-mini to 72.8% on grok-4.6, same commit, same prompt, same cases. Lever 4 dominates: one point of pass rate is worth US$608 a month, seven times the entire token bill.

Across plus or minus 10 points of pass rate, cost per successful task spans US$11.15 to US$4.96 and the conclusion holds throughout - accuracy dominates at every point in the range.

Caps shipped with it: an 8-turn step cap and a 60,000-token budget ceiling, both enforced in Guardrails; and 200 claims per account per month, which is a policy commitment at provisioning, not a code control, because a per-run guard cannot see cross-run usage. Baseline excludes caching and reasoning-model adjustments by design."""
